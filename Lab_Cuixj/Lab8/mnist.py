...
# dataset parameters
parser.add_argument('--data_path', type=str, default='./data/mnist1/', help='path to dataset')
...
def train(model, type):
    ...
    train_set = MNISTDataset(root_dir='data/mnist_jpg', mode='training')
    test_set = MNISTDataset(root_dir='data/mnist_jpg', mode='test')   
    ...
    save_dir = "MNIST_training_results"
    ...
    return train_loss, train_acc, train_f1, acc / test_set.__len__()

if __name__ == "__main__":
    cnn_model = CNN(in_channels = 1, num_classes = 10, input_dim = 4 * 4, hidden_dim = 64)
    mlp_model = MLP(input_dim = 28 * 28, hidden_dim = (256, 128, 64, 10))
    cnn_loss, cnn_acc, cnn_f1, cnn_test_acc = train(cnn_model, 'cnn')
    mlp_loss, mlp_acc, mlp_f1, mlp_test_acc = train(mlp_model, 'mlp')
    # -----------------------------可视化及保存------------------------------------------
    ...
    filename = f"mnist_comparison.png"
    save_dir = "MNIST_training_results"
