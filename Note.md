...
- 二者关系与计算：
  $$
  V_{\pi}(s) = \sum_{a \in A} \pi(a|s) Q_{\pi}(s, a) = \sum_{a \in A} \pi(a|s)\big( R^a_s + \gamma \sum_{s' \in \mathcal{S}} p^a_{ss'} V_{\pi}(s') \big)\\
...
- 最优状态函数：状态 $s$ 的最优价值 $V_*(s)$ 等于：在所有可能的动作 $a$ 中**==选择一个动作==**，使得这个动作带来的即时奖励 $R_s^a$ 加上**折扣后（$\gamma$）的、从 $s$ 执行 $a$ 后转移到所有可能后续状态 $s'$ 的最优价值 $V_(s')$ 的加权平均（由状态转移概率 $P_{ss'}^a$ 加权）**的总和达到最大。
- 最优动作函数：在状态 $s$ 下执行动作 $a$ 的最优价值 $Q_*(s, a)$ 等于：
  1. 立即获得的奖励 $R_s^a$
  2. 加上折扣因子 $\gamma$ 乘以，从 $(s, a)$ 出发转移到所有可能后续状态 $s'$ 的转移概率 $P_{ss'}^a$ 的加权平均。
  3. 对于每个转移到的后续状态 $s'$，我们取在该状态下**所有可能动作 $a'$ 中最优动作价值 $\max_{a'} Q_*(s', a')$** 的值。
