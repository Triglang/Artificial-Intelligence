...
* 因此贝叶斯推断可理解为：
  ...
  $$
  贝叶斯推断含义：我们先预估一个先验概率，然后加入实验结果，看这个实验到底是增强还是削弱了先验概率，由此得到更接近事实的后验概率。
...
4. D-分离：
   ...
   定义：若 $ Z $ 阻塞了节点 $ X $ 和节点 $ Y $ 之间的每一条路径，则称给定 $ Z $ 时，$ X $ 和 $ Y $ 是 D-分离的，即给定 $ Z $ 时，$ X $ 和 $ Y $ 条件独立。
